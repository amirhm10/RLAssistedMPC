...
- For Markdown reports with equations, write display math in renderer-friendly LaTeX. Prefer one-line `$$ ... $$` blocks for single equations, for example `$$ \Delta R_{\mathrm{release}} = \bar{R}_{\mathrm{live},1{:}10} - \bar{R}_{\mathrm{warm},\mathrm{tail}}. $$`. Use `\begin{aligned}...\end{aligned}` only when there are multiple aligned equations, keep each relation's operator and right-hand side on the same visual line, use `\\` for line breaks, and use `\mathrm{...}` for text-like subscripts or superscripts such as `cand`, `nom`, `phys`, `raw`, and `eff`. Avoid loose multi-line `$$` blocks with separate operator-only lines such as `=`, `&=`, `-`, or equations joined only by `\qquad`. Avoid multi-line expectation/bracket blocks such as `\mathbb{E}...\left[ ... \right]` and continuation-only rows like `&\quad + ...`; instead write expectations on one line with `[ ... ]`, or split long objectives into named one-line terms such as `\mathcal{L}_Q`, `\mathcal{L}_{\mathrm{BC}}`, and `\mathcal{L}_{\mathrm{actor}}`.
