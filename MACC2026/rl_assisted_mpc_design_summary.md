...
The project was successful in both polymer and distillation. In both case studies, the RL-assisted MPC families improved the overall controller performance compared with using a fixed nominal MPC design alone:

| Case | Successful outcome |
|---|---|
| Polymer | The RL-assisted MPC methods improved performance over fixed MPC by adapting model multipliers, residual corrections, weights, and horizons around the same constrained MPC layer. |
| Distillation | The RL-assisted MPC methods improved performance over fixed MPC by adapting the controller to the coupled column dynamics; the residual, matrix, weight, and horizon families showed that learned supervision can improve tracking and input behavior when interpreted across the available reward and physical-performance metrics. |
...
This is a useful result for process-control research. The project does not claim that a single RL agent should replace MPC. It shows that MPC can be made adaptive by using RL agents as interpretable supervisors. In both systems, the learned supervisors improved the practical controller design by changing the parts of MPC that matter most for the current operating condition.
